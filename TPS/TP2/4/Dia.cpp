...
Hitos::Hitos(string titulo,Fecha fec, string descripcion, int imp){
    //No verifico nada del titulo salu2
    this->Titulo=titulo;
    //Fecha se verifica sola por todo lo anterior
    this->v_fecha = fec;
    //No verifique el titulo de imaginaras que pasa con esto...
    this->Descripcion = descripcion;
    //importancia que sea lo que quieran, no tengo problema con que tenga varios Hitos con igual importancia
    this->importancia = imp;
}

bool Hitos::operator==(Hitos a){
    if(this->importancia == a.getImportancia()){
        return true;
    }
    return false;
}
...
ostream& operator<<(ostream& os, Hitos& a) {
    os << "El Hito es: " << a.getTitulo() <<endl<< "Sucedio el:" << a.getFecha() <<endl<<"Descripcion: "<< a.getDescripcion() <<endl;
    return os;
}

void Inserta_ordenado(list<Hitos>& List , Hitos& a){
    for( auto x = List.begin( ); x != List.end( ); ++x){
        //cout<<(*x)<<endl;
        if ((*x).getFecha()< a.getFecha())
        {
            List.insert(x,a);
        }else if ((*x).getFecha()== a.getFecha())
        {
            if ((*x).getImportancia()> a.getImportancia())
            {
                List.insert(x,a);
            }
...
// function for printing the elements in Hitos list in reverse
void showlist(list<Hitos> g)
{
